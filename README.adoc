...
| v1.4   | EOGS      | https://docs.vmware.com/en/VMware-Spring-Cloud-Data-Flow-for-Kubernetes/1.4/scdf-k8s/GUID-index.html |
...
2. Enter the complete path to the docs content repository on your local machine. For example, `{example-content-repository-path}`.

3. Type `n`, then press Enter.

4. Press any key.

5. Select `Back`, then press Enter.

6. Select `Exit`, then press Enter.

7. In your IDE or text editor, open the newly-generated file `_md-cli/dist/config.json`.

8. Set `tocLocation` to `toc.md`.

9. Set `variablesLocation` to `variables.yml`.
